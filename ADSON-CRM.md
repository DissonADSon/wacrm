...
### ✨ Melhorias reportadas pela Michele (17/07 — `JOH-20260717-1131-T5A`) — ✅ DEPLOYADO (1.1.4, img `6a48dae2`)
> 5 "ideias/detalhe" enviadas pelo canal de erros. Corrigidas com fix mínimo (ponytail), commit `f2ec93b` (branch `feat/johari-t5a-ux`). `typecheck` + `next build` + `eslint` (0 erros) + `vitest currency` verdes. **Verificado no navegador** (conta de teste isolada, seed+cleanup): todos os 5 provados; console 0 erros, network 0 request 4xx/5xx. **Deploy 17/07** via `PUT /api/stacks/43` (start-first, ZERO downtime, sem rollback; Johari+Demo 200 o tempo todo). Rollback: img `sha256:9cc416c7`. Registro Supabase `uid=217ee32a…` → `resolvido`. Roadmap do portal atualizado (⚠️ replace apagou sectionNote/footerNote — re-adicionar no portal).
- [x] **#1 Filtrar por "recentes" e "sem tag" (inbox + contatos).** Inbox (`conversation-list.tsx`): opção **"Recentes (7 dias)"** no dropdown de estado (janela por `last_message_at`, corte calculado no clique p/ não violar `react-hooks/purity`) + **"Sem tag"** no dropdown de tags (`tagsOf(c).length===0`). Contatos (`contacts/page.tsx`): botão-toggle **"Recentes"** (ordena por `created_at desc`) + **"Sem tag"** no dropdown de tags (server-side, inverte o padrão `.in(ids)` para `.not('id','in',(...))`). Zero componente/dep novos.
- [x] **#2 Valores com casa decimal (arredondava).** Não era input nem banco (`deals.value` é `NUMERIC(12,2)`) — era a exibição. `lib/currency.ts` `formatCurrency` fixava `min/maxFractionDigits:0`. **Fix:** usar as casas naturais do Intl (BRL/USD→2, JPY→0); fallback de código inválido → 2 casas. Conserta deal-card, board, analytics, dashboard, contact-detail de uma vez. `+ step="0.01"/inputMode="decimal"` no input do valor (`deal-form.tsx`). Teste `currency.test.ts` atualizado. (Obs.: agrupamento `.`/`,` segue o locale do navegador — no pt-BR do cliente sai "R$ 1.234,56".)
- [x] **#3 Ir direto pra conversa pelo funil.** O link "Ir para a conversa" no `deal-form.tsx` tinha `href="/inbox"` fixo (caía na caixa genérica). **Fix:** `href={`/inbox?c=${linkedConversation.id}`}` — deep-link que a inbox já suporta (`inbox/page.tsx:27` lê `?c=`). Provado: landing auto-seleciona a conversa certa.
- [x] **#4 Botão "+ Adicionar oportunidade" em cima.** Estava no rodapé da coluna (`pipeline-board.tsx`, `StageColumn`). **Fix:** reordenado o JSX pro topo do corpo da coluna (abaixo do título/valor, acima dos cards). DnD intacto (só a região dos cards é droppable). Provado: `btnTop < cardTop`.
- [x] **#5 Editar tags direto na conversa.** A seção Tags do `contact-sidebar.tsx` (inbox) era só-leitura. **Fix:** botão **"Editar"** revela lista togglável de tags (portou o padrão de `contact-detail-view.tsx`: insert/delete em `contact_tags`). Provado: add→201, remove→204, persiste no banco.
