...
### ✨ Melhorias reportadas pela Michele (17/07 — `JOH-20260717-1131-T5A`) — ✅ FEITO no código + VALIDADO, ⏳ aguardando deploy noturno
> 5 "ideias/detalhe" enviadas pelo canal de erros. Corrigidas/implementadas com fix mínimo (ponytail), na branch `fix/johari-call-30jun`. `typecheck` + `next build` + `eslint` (0 erros) + `vitest currency` verdes. **Verificado no navegador** (conta de teste isolada, seed+cleanup): todos os 5 provados; console 0 erros, network 0 request 4xx/5xx.
