...
const STEP_TYPE_LABELS: Record<AutomationLogStepResult["step_type"], string> = {
  ...
  create_deal: "Criar oportunidade",
  wait: "Aguardar",
  condition: "Condição",
...
function StepRow({ result }: { result: AutomationLogStepResult }) {
  ...
  return (
    <li className="flex items-start gap-2 text-xs">
      ...
      <span className="text-muted-foreground">{STEP_TYPE_LABELS[result.step_type] ?? result.step_type}</span>
      {result.detail && (
