    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-border bg-card/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        <Metric
          ...
          tooltip="Contagem de todas as oportunidades deste pipeline que não estão marcadas como Perdidas. Oportunidades ganhas continuam incluídas."
        />
        <Metric
          icon={<DollarSign className="h-4 w-4 text-primary" />}
          label="Valor do pipeline"
          value={formatCurrency(stats.totalValue, defaultCurrency)}
          tooltip="Soma dos valores de todas as oportunidades deste pipeline, excluindo as marcadas como Perdidas."
        />
        <Metric
          icon={<Target className="h-4 w-4 text-blue-400" />}
          label="Valor médio"
          value={formatCurrency(stats.avgValue, defaultCurrency)}
          tooltip="Valor do pipeline dividido pelo total de oportunidades — o valor médio de uma oportunidade não perdida."
        />
        <Metric
          icon={<TrendingUp className="h-4 w-4 text-purple-400" />}
          label="Valor ponderado"
          value={formatCurrency(stats.weightedValue, defaultCurrency)}
          tooltip="Receita esperada: o valor de cada oportunidade em aberto × a probabilidade da sua etapa. Primeira etapa ≈ 10%, as etapas avançam até 90%, Ganha = 100%. Oportunidades perdidas são excluídas."
        />
        <Metric
