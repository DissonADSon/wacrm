...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? `nossa conta no ${account?.brand_name || APP_NAME}`,
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const brand = account?.brand_name || APP_NAME;
    const accountName = result?.accountName ?? `nossa conta no ${brand}`;
    const message = `Entre em ${accountName} no ${brand} usando este link (válido por ${result?.expiresInDays} dias): ${url}`;
        ...
        {result ? (
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-xs text-amber-200">
                ...
                Nunca guardamos o texto puro — assim que você fechar esta
                janela, a URL desaparece. Para compartilhar de novo, revogue
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Gere um link de convite de uso único. Compartilhe pelo
                WhatsApp, Slack ou qualquer canal — sem precisar de serviço
                de e-mail.
              </DialogDescription>
            </DialogHeader>
