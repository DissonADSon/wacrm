...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      const nextRoleLabel =
        EDITABLE_ROLES.find((r) => r.value === nextRole)?.label ?? nextRole;
      toast.success(`${member.full_name || 'Membro'} atualizado para ${nextRoleLabel}`);
    } catch (err) {
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              Por segurança, a URL do convite em texto puro é exibida apenas
              uma vez, na criação — para compartilhar de novo, revogue o
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              conta pessoal no próximo login. O login dele não é excluído.
            </DialogDescription>
          </DialogHeader>
