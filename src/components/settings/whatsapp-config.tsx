...
// Modo EvoHub: o WhatsApp é conectado/gerenciado via gateway (proxy Meta),
// então o cliente NÃO precisa fazer o setup Meta-direto (app, PIN, webhook,
// token de verificação). Quando ligado, a tela mostra só o status da conexão.
const EVOHUB_MODE = process.env.NEXT_PUBLIC_WHATSAPP_PROVIDER === 'evohub';
...
export function WhatsAppConfig() {
  ...
  // Nome verificado do número (ex.: "Johari - Desde 1988"), só p/ exibição no modo EvoHub.
      ...
      if (data) {
        try {
          ...
          if (payload.connected) {
            ...
            setResetReason(null);
            setStatusMessage('');
            setPhoneName(payload.phone_info?.verified_name || '');
          } else {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error('Reinforme o token de acesso para salvar as alterações');
        setSaving(false);
        return;
      }
  ...
  if (EVOHUB_MODE) {
    ...
    return (
      <section className="animate-in fade-in-50 duration-200">
        <SettingsPanelHead
          title="Conexão do WhatsApp"
          description="Seu WhatsApp Business está conectado e gerenciado para você. Aqui você acompanha o status da conexão."
        />
        <div className="space-y-6 max-w-2xl">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <AlertTitle
                className={'mb-0 ' + (connected ? 'text-emerald-200' : 'text-foreground')}
              >
                {connected ? 'WhatsApp conectado' : 'Verificando conexão…'}
              </AlertTitle>
            </div>
            <AlertDescription className="text-muted-foreground mt-1">
              {connected
                ? `${phoneName ? phoneName + ' — ' : ''}recebendo e enviando mensagens normalmente.`
                : 'Aguarde um instante enquanto confirmamos a conexão com o WhatsApp.'}
            </AlertDescription>
          </Alert>

          <Card>
            <CardHeader>
              <CardTitle className="text-foreground">Número conectado</CardTitle>
              <CardDescription className="text-muted-foreground">
                Conexão gerenciada pela ADSon — você não precisa configurar nada aqui.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label className="text-muted-foreground">ID do número</Label>
                <Input
                  readOnly
                  value={phoneNumberId}
          ...
          >
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div className="flex items-center gap-2">
                ...
                >
                  {isRegistered
                    ? `Registrado — a Meta entregará eventos ao ${APP_NAME}`
            ...
            <AlertDescription className="text-muted-foreground mt-2 text-xs leading-relaxed">
              {isRegistered ? (
                ...
                <>
                  Este número foi salvo antes de o controle de registro
                  existir, ou o registro foi ignorado. Informe o PIN de
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {config && !tokenEdited && (
                <p className="text-xs text-muted-foreground">
                  O token fica oculto por segurança. Reinforme-o para atualizar a configuração.
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                ...
                , depois cole aqui para que o {APP_NAME} possa inscrever o número —
                caso contrário, a Meta roteia os eventos de entrada para o
                último app que reivindicou o número (o sintoma que afeta
                segundos números sob uma WABA compartilhada).{' '}
              ...
              <AccordionItem className="border-border">
                ...
                <AccordionContent className="text-muted-foreground">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>Informe o mesmo <strong className="text-foreground">Token de Verificação</strong> que você definiu aqui</li>
