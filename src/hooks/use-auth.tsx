...
interface AccountSummary {
  ...
  /** Marca white-label desta conta (migration 024). NULL = usa o
   *  default da instância (APP_NAME). Exibida na UI pós-login. */
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          ...
          "id, full_name, email, avatar_url, role, beta_features, account_id, account_role, account:accounts!inner(id, name, default_currency, brand_name)",
        )
        .eq("user_id", userId)
        .maybeSingle();
      ...
      if (data) {
        ...
        const accountRow: AccountSummary | null = accountRaw
          ? {
              ...
              brand_name: accountRaw.brand_name ?? null,
            }
          : null;
