  ...
  switch (step.step_type) {
    ...
    case 'assign_conversation':
      if (c.mode === 'specific' && !nonEmpty(c.agent_id)) {
        issues.push({
          path: `${path}.agent_id`,
          message: 'o agente é obrigatório quando o modo é "specific"',
        })
      }
      break
    ...
    case 'wait':
      ...
      if (!['minutes', 'hours', 'days'].includes(String(c.unit))) {
        issues.push({
          path: `${path}.unit`,
          message: 'a unidade de espera deve ser minutes, hours ou days',
        })
      }
      break
    case 'condition':
      if (!nonEmpty(c.subject)) {
        issues.push({ path: `${path}.subject`, message: 'o sujeito da condição é obrigatório' })
      }
    ...
    case 'send_webhook':
      ...
      try {
        ...
        issues.push({ path: `${path}.url`, message: 'a URL do webhook não é uma URL válida' })
      }
      break
...
export function validateTriggerForActivation(
  ...
  if (triggerType === 'keyword_match') {
    ...
    if (!Array.isArray(k) || k.length === 0) {
      ...
      issues.push({ path: 'trigger.keywords', message: 'palavras-chave não podem ser strings vazias' })
    }
