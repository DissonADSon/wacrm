...
describe("validateFlowForActivation — nodes", () => {
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "b" &&
          i.field === "buttons" &&
          i.message.includes("no máximo 3 botões"),
      ),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          ...
          i.message.includes("passa de 20"),
      ),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "l" &&
          i.field === "sections" &&
          i.message.includes("no máximo 10 linhas"),
      ),
    ).toBe(true);
  });
  ...
  it("flags list row title over 24 chars", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("excede 24")),
    ).toBe(true);
  });
    ...
    expect(
      issues.some(
        (i) =>
          i.node_key === "orphan" &&
          i.severity === "warning" &&
          i.message.includes("inalcançável"),
      ),
    ).toBe(true);
  });
